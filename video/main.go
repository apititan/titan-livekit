...
func main() {
	...
	r.Handle("/video/{chatId}/notify/{streamId}", http.HandlerFunc(handler.NotifyChatParticipants)).Methods("PUT")
	...
	r.Handle("/video/{chatId}/user-info/{streamId}", http.HandlerFunc(handler.User)).Methods("GET")
